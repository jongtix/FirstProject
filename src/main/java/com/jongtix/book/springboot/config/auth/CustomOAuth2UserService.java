...
import com.jongtix.book.springboot.config.auth.dto.OAuthAttributes;
import com.jongtix.book.springboot.config.auth.dto.SessionUser;
import com.jongtix.book.springboot.domain.user.User;
import com.jongtix.book.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String registrationId = userRequest.getClientRegistration().getRegistrationId();    //현재 로그인 진행 중인 서비스를 구분하는 코드
                                                                                            //향후 구글인지 네이버인지 등을 구분
        String userNameAttributeName    //userNameAttributeName: OAuth2 로그인 진행 시 키가 되는 필드값
                                        //구글의 기본 코드는 sub, 네이버 카카오 등은 기본 지원하지 않음
